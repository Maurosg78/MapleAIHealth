...
interface NoteInputProps {
    onSubmit: (note: {
        content: string;
        type: 'consultation' | 'emergency' | 'follow-up' | 'lab-result' | 'prescription' | 'other';
        timestamp: string;
        author: string;
    }) => void;
}
...
export const NoteInput: React.FC<NoteInputProps> = ({ onSubmit }) => {
    ...
    const [type, setType] = useState<'consultation' | 'emergency' | 'follow-up' | 'lab-result' | 'prescription' | 'other'>('consultation');
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                ...
                <Select
                    id="note-type"
                    value={type}
                    onChange={(e: ChangeEvent<HTMLSelectElement>) => setType(e.target.value as typeof type)}
                    className="mt-1"
                    options={noteTypes}
                />
            </div>
