...
import { AIResponse, TimelineEvent, Insight, Recommendation } from '../../services/ai/types';
...
const AnalysisResults: React.FC<AnalysisResultsProps> = ({ response }) => {
    const renderTimeline = (timeline: { date: string; events: TimelineEvent[] }) => (
            ...
            <CardContent>
                ...
                {response.insights && response.insights.length > 0 && (
                    <>
                        ...
                        <List>
                            {response.insights.map(renderInsights)}
                        </List>
                        <Divider sx={{ my: 2 }} />
                    </>
                )}
                ...
                {response.recommendations && response.recommendations.length > 0 && (
                    <>
                        ...
                        <List>
                            {response.recommendations.map(renderRecommendations)}
                        </List>
                    </>
                )}
            </CardContent>
