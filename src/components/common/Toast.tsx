...
export const Toast: React.FC<ToastProps> = ({
    ...
    const baseStyles = 'fixed p-4 rounded-lg shadow-lg max-w-sm flex items-center gap-3';
    const typeStyles = {
    ...
    return createPortal(
        <div
            className={cn(baseStyles, typeStyles[type], positions[position], className)}
            role="alert"
            aria-live="polite"
        >
            {Icon && <Icon className={cn('h-5 w-5', iconColorStyles[type])} aria-hidden="true" />}
