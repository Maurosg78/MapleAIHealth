...
describe('AIService', () => {
    ...
    it('should detect contradictions in medical data', async () => {
        ...
        const emrData: EMRData = {
            patientId: 'test123',
            medicalHistory: [
                { date: '2023-01-01', type: 'note', description: 'Initial consultation' }
            ],
            medications: [
        ...
        vi.mocked(cacheService.get).mockResolvedValue(null);
        vi.mocked(cacheService.set).mockResolvedValue(undefined);
        vi.spyOn(aiService as unknown as AIServiceInternals, 'getEMRData').mockResolvedValue(emrData);
        vi.spyOn(aiService as unknown as AIServiceInternals, 'generateSimulatedResponse').mockResolvedValue({
        ...
        expect(result.insights?.some(insight => insight.type === 'contradiction')).toBe(true);
    });
    ...
    it('should retry operations on failure', async () => {
        ...
        const result = await (aiService as unknown as AIServiceInternals).executeWithRetry(mockOperation);
    ...
    it('should throw AIServiceError on unrecoverable errors', async () => {
        ...
        vi.spyOn(aiService as unknown as AIServiceInternals, 'generateSimulatedResponse').mockRejectedValue(new Error('Simulated API error'));
