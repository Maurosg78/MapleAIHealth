...
export interface ICacheService {
  ...
  set: (query: string, response: AIResponse, metadata?: { provider: string; cost: number; processingTime: number; }) => Promise<void>;
}
...
type CapabilityType = 'emr-analysis' | 'timeline-organization' | 'insight-detection' | 'clinical-evidence' | 'treatment-patterns';
...
export class AIService {
    ...
    private readonly providers: AIProvider[] = [
        {
            ...
            name: 'GPT-4 Medical',
            costPerQuery: 0.03,
            capabilities: ['emr-analysis', 'timeline-organization', 'insight-detection'] as CapabilityType[]
        },
        {
            id: 'med-palm-2',
            name: 'Med-PaLM 2',
            costPerQuery: 0.05,
            capabilities: ['emr-analysis', 'clinical-evidence', 'treatment-patterns'] as CapabilityType[]
        }
    ];
    ...
    private setupProviders(): void {
        ...
        if (googleProjectId) {
            try {
                ...
                this.logger.error('Failed to configure Google MedPaLM provider', { error });
            }
        }
    }
    ...
    async analyzeUnstructuredNotes(patientId: string, notes: UnstructuredNote[]): Promise<AIResponse> {
        this.logger.info('Analyzing unstructured notes', { patientId, noteCount: notes.length });
        ...
        try {
            ...
            if (contradictions.length > 0) {
                this.logger.warn('Contradictions detected', { count: contradictions.length });
            ...
            const metadata = {
                ...
                cost: this.estimateCost(this.config.defaultProvider ?? 'gpt-4-medical', 1),
                processingTime
            };
            ...
            this.logger.error('Error analyzing unstructured notes', { error, patientId });
            throw new AIServiceError('Error al analizar las notas médicas', error as Error);
        }
    }
    ...
    private async getEMRData(patientId: string): Promise<EMRData> {
        try {
            ...
            throw new AIServiceError('Error al obtener datos del historial médico', error as Error);
        }
    }
     ...
     */
    private async executeWithRetry<T>(operation: () => Promise<T>, retries = this.MAX_RETRIES): Promise<T> {
        try {
            return await operation();
        } catch (error) {
            if (retries > 0) {
                this.logger.warn(`Operation failed, retrying... (${this.MAX_RETRIES - retries + 1}/${this.MAX_RETRIES})`, { error });
     ...
     */
    private detectContradictions(emrData: EMRData, notes: UnstructuredNote[]): Insight[] {
        this.logger.debug('Detecting contradictions', { emrDataAvailable: !!emrData, notesCount: notes.length });
        const contradictions: Insight[] = [];

        // Contradicciones en medicamentos
        const emrMedications = new Set(emrData.medications.map(med => med.name.toLowerCase()));

        // Medicamentos mencionados en las notas
        const noteMedications = new Set<string>();
        notes.forEach(note => {
            note.medications?.forEach(med => noteMedications.add(med.toLowerCase()));
        });

        // Contradicciones en medicamentos
        if (noteMedications.size > 0) {
            const noteMedsNotInEMR = Array.from(noteMedications).filter(med => !emrMedications.has(med));

            if (noteMedsNotInEMR.length > 0) {
                contradictions.push({
                    type: 'contradiction' as InsightType,
                    description: 'Medicamentos mencionados en notas que no aparecen en el historial médico',
                    severity: 'medium',
                    evidence: noteMedsNotInEMR.map(med => `Medicamento: ${med}`)
                });
                this.logger.debug('Medication contradiction detected', { medications: noteMedsNotInEMR });
            }
        }
        ...
        if (contradictoryDiagnosis.size > 0) {
            contradictions.push({
                type: 'contradiction' as InsightType,
                description: 'Posibles diagnósticos contradictorios encontrados en diferentes notas',
                severity: 'high',
                evidence: Array.from(contradictoryDiagnosis).map(d => `Diagnóstico: ${d}`)
            });
            this.logger.debug('Diagnosis contradiction detected', { diagnoses: Array.from(contradictoryDiagnosis) });
        }
    ...
    private generateInsights(response: AIResponse, emrData: EMRData): Insight[] {
        ...
        if (response.timeline) {
            ...
            if (timelineGaps.length > 0) {
                ...
                this.logger.debug('Timeline gaps detected', { count: timelineGaps.length });
            }
        }
    ...
    private analyzeTimelineGaps(timeline: { date: string; events: TimelineEvent[]; }[]): Insight[] {
        const insights: Insight[] = [];
        ...
        for (let i = 1; i < sortedTimeline.length; i++) {
            const gap = new Date(sortedTimeline[i].date).getTime() -
                       new Date(sortedTimeline[i-1].date).getTime();

            if (gap > 30 * 24 * 60 * 60 * 1000) { // Más de 30 días
                insights.push({
    ...
    private analyzeRiskFactors(emrData: EMRData): Insight[] {
        ...
        const riskKeywords = ['riesgo', 'complicación', 'adverso', 'efecto secundario', 'contraindicación'];
        const riskNotes = emrData.medicalHistory
            .filter(record => record.type === 'note')
            .filter(note =>
                riskKeywords.some(keyword => note.description.toLowerCase().includes(keyword))
            );
        ...
        if (riskNotes.length > 0 || medicationsWithRisks.length > 0) {
            insights.push({
                ...
                description: 'Se detectaron factores de riesgo en las notas médicas y medicamentos',
                severity: 'high',
                evidence: [
                    ...riskNotes.map(note => note.description),
                    ...medicationsWithRisks.map(med => `Medicamento con riesgo: ${med.name}`)
                ]
            });
        }
    ...
    async query(query: AIQuery): Promise<AIResponse> {
        ...
        try {
            ...
            const providerId = query.providerId ?? this.config.defaultProvider ?? 'gpt-4-medical';
            const startTime = Date.now();

            // Procesar la consulta con el proveedor adecuado
            const response = await this.executeWithRetry(() => this.processWithProvider(providerId, query));

            const processingTime = Date.now() - startTime;
            this.logger.debug('AI processing complete', { processingTime, providerId });
            ...
            throw new AIServiceError('Error al procesar la consulta de IA', error as Error);
        }
    }

    private async processWithProvider(providerId: string, query: AIQuery): Promise<AIResponse> {
        ...
        if (!provider) {
            this.logger.warn(`Provider ${providerId} not found, trying default provider`, {
    ...
    async analyzeEMRData(emrData: EMRData): Promise<AIResponse> {
        ...
        try {
            ...
            this.logger.error('Error analyzing EMR data', { error, patientId: emrData.patientId });
            throw new AIServiceError('Error al analizar los datos del EMR', error as Error);
        }
    }
    ...
    async analyzeUnstructuredNote(note: UnstructuredNote): Promise<AIResponse> {
        ...
        try {
            return await this.query({
                query: 'Analiza la siguiente nota médica y extrae información relevante',
                unstructuredNotes: [note]
            });
        } catch (error) {
            this.logger.error('Error analyzing unstructured note', { error, noteType: note.type });
            throw new AIServiceError('Error al analizar la nota médica', error as Error);
        }
    }
                        ...
                        timeline: [
                            ...
                            },
                            {
                                ...
                                events: [
                                    {
                                        type: 'lab-result',
                                        description: 'Resultados de resonancia magnética: desgarro de menisco',
                        ...
                        insights: [
                            {
                                ...
                                description: 'El paciente no ha tenido seguimiento después del diagnóstico de desgarro de menisco',
                                severity: 'medium',
                                evidence: ['Última visita: 10/03/2023', 'No hay registros de seguimiento para el plan de tratamiento'],
                                recommendation: 'Programar consulta de seguimiento con ortopedia'
                            }
                        ]
                    });
                } else {
                    resolve({
                        answer: 'He analizado la consulta pero necesito más contexto para proporcionar insights detallados.',
                        confidence: 0.75
                    });
                }
