...
export class OpenAIProvider implements AIProviderClient {
  ...
  readonly capabilities = ['emr-analysis', 'timeline-organization', 'insight-detection'];
  ...
  async processQuery(query: AIQuery): Promise<AIResponse> {
    ...
    try {
      this.logger.debug('Processing query with OpenAI', { queryType: query.context?.type });
      ...
      throw new Error(`Error al procesar consulta con OpenAI: ${(error as Error).message}`);
    }
  }

  private buildMessages(query: AIQuery): Array<{ role: string; content: string }> {
...
Para la clave "insights", cada elemento debe incluir "type", "description", "severity", "evidence" y opcionalmente "recommendation".`;
  ...
  private async callOpenAI(messages: Array<{ role: string; content: string }>): Promise<OpenAIResponse> {
