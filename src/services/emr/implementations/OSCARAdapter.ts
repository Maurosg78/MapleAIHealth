...
export class OSCARAdapter extends GenericEMRAdapter implements EMRAdapter {
  ...
  private validateOSCARConfig(config: EMRAdapterConfig): void {
    if (!config.baseUrl) {
      console.warn('OSCAR Adapter: baseUrl no especificada, usando valor por defecto');
    }
  }
  ...
  async testConnection(): Promise<boolean> {
    ...
    console.log('Verificando conexión con OSCAR en:', this.config.baseUrl);
  ...
  async getPatientData(patientId: string): Promise<PatientData> {
    ...
    console.log('Obteniendo datos del paciente desde OSCAR:', patientId);
  ...
  async getUnstructuredNotes(patientId: string, limit = 10): Promise<EMRUnstructuredNote[]> {
    ...
    // Por ahora usamos datos simulados pero con formato específico de OSCAR
    console.log('Obteniendo notas médicas desde OSCAR:', patientId);

    // Simulamos un retraso de red
    await new Promise(resolve => setTimeout(resolve, 300));

    // Notas en formato OSCAR (simulado)
  ...
  async getCompleteEMRData(patientId: string): Promise<CompleteEMRData> {
    ...
    console.log('Obteniendo datos completos desde OSCAR:', patientId);
