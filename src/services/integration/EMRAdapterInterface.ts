...
export interface EMRAdapterInterface {
   ...
   */
  getRecentNotes(patientId: string, limit?: number, days?: number): Promise<UnstructuredNote[]>;
   ...
   */
  updatePatientRecord(patientId: string, data: Partial<EMRData>): Promise<boolean>;
   ...
   */
  syncTreatments(patientId: string, treatments: Array<{
    name: string;
    dosage?: string;
    frequency?: string;
    duration?: string;
   ...
   */
  getInteractionHistory(patientId: string, providerId: string): Promise<Array<{
    date: string;
    type: string;
    summary: string;
    aiAssisted: boolean;
